...
class DroneManager(object):
    def __init__(self, host_ip='192.168.10.2', host_port=8889, drone_ip='192.168.10.1', drone_port=8889, is_imperial=False, speed=DEFAULT_SPEED):
    ...
    def receive_response(self, stop_event):
        while not stop_event.is_set():
            try:
                ...
                logger.error({'action': 'receive_reponse', 'ex': ex})
                break
    ...
    def send_command(self, command):
        ...
        if self.response is None:
            ...
            reponse = self.response.decode('utf-8')
        self.response = None
        return response
